# Dissertation — the defended artifact

**Title:** *Adolescent Neurocognitive Maturity Mediates Paths to Altered Social Norms & Vulnerability
in Emerging Adulthood*
**Author:** Shady El Damaty · Interdisciplinary Program in Neuroscience, Georgetown University
**Defended:** 17 May 2017
**Advisor:** John W. VanMeter (now Ashley S. VanMeter; she/her) · **Co-mentors:** Diana H. Fishbein (Penn State), Emma J. Rose
**Committee:** Chandan Vaidya (chair), Jennifer Woolard, Mary Ann Dutton
**Funding:** NIJ 2016-R2-CX-0019 (OJP 2016-90111-DC-IJ); NIH/NIAAA R01AA019983; NIH/NCATS KL2RR031974; NIH/NICHD P30HD040677

This is the umbrella output of the CLAD study. It `aggregates` the component papers (see [`../papers/`](../papers/)):
- **CMI** — *Introducing an Adolescent Cognitive Maturity Index* → `eldamaty2020b`, **published**
  (Frontiers in Psychology 2022, [doi:10.3389/fpsyg.2022.1017317](https://doi.org/10.3389/fpsyg.2022.1017317)). RQ1.
- **Violence cascade** — social-strain → altered norms → violence SEM → `eldamaty2020a`, draft. RQ2.
- **Striatal parcellation** — multimodal striatal functional/diffusion parcellation → `eldamaty2020c`, draft. RQ3.

Graph node: `pub-eldamaty-2017-dissertation` (in [`../graph/publications/`](../graph/publications/)).

## Contents
- [`defense/`](defense/) — committee/defense paperwork (signed meeting form, proposal & reviewer forms,
  agendas, post-meeting form). PHI-free faculty documents.
- [`monograph/`](monograph/) — the full dissertation PDF: `El_Damaty__Dissertation_2020.pdf` (154 pp,
  26.9 MB; pdfTeX-typeset). Recovered from the NeuroYouths Box (root). Public ProQuest-deposited monograph,
  PHI-free. Large binaries are managed by **DataLad + git-annex** (gitbare siblings + encrypted special remote, per `docs/data-governance.md`). The monograph is also committed directly to this GitHub mirror (26.9 MB, within limits); `*.pdf >5mb` is annex-routed in `.gitattributes` for the canonical DataLad dataset. **Reported sample:** 141 enrolled (6 excluded at
  baseline); per-wave imaging N 141/106/97; post-QC unique T1w 140/106/100; the striatal parcellation (RQ3)
  is built on Wave-1 resting-state only (9 stable bilateral parcels).

## Deferred (large binary)
- **`17May_Slides.pptx`** — the 63 MB defense presentation. Not committed in the skeleton pass; will be
  annexed in the data-migration pass. Source:
  `~/ads-glimmer-staging/neuroyouths-recovery/dissertation/17May_Slides.pptx`
  · sha256 `573d8606fa2c38d8b5c30fcdaa3ff29297b932a67523f0c84fd3f796124f48e8` · 65,769,830 bytes.
